...
class RouteService:
    ...
    def __init__(self, node: Node) -> None:
        ...
        self.mission_start_distance: float = 0.0;
    ...
    def __mission_flush(self) -> None:
        ...
        self.__log.info("====================== Mission Flushed ======================{self.mission_start_distance}");
    ...
    def start_mission(self) -> None:
        if self.__is_goal_need_to_retry == False:
            ...
            self.__log.info(f"{ROUTE_TO_POSE_ACTION_NAME} mission_start_distance : {self.mission_start_distance}");
                            
            self.__status_service.notify_mission_status_publish(status="Started");
            self.__log.info(f"==================================== Mission Started ===================================={self.mission_start_distance}");
        else:
            return;
    ...
    def end_mission(self) -> None:
        ...
        self.__log.info(f"==================================== Mission End ===================================={self.mission_start_distance}");
        self.mission_start_distance = 0.0;
    ...
    def process_return(self) -> None:
        ...
        self.__goal_index = 0;
        self.__log.info(f"==================================== Returning Finished ===================================={self.mission_start_distance}");
        self.end_mission();
        self.__goal_flush();
    ...
    def process_no_return(self) -> None:
        ...
        self.__goal_index = 0;
        self.__log.info(f"==================================== No Return ===================================={self.mission_start_distance}");
        self.end_mission();
        self.__goal_flush();
        self.__mission_flush();
    ...
    def __process_cancel_no_return(self) -> None:
        ...
        set_driving_flag(flag=False);
        set_driving_status(driving_status=DRIVE_STATUS_WAIT);
        self.__log.info(f"==================================== Cancel No Return ===================================={self.mission_start_distance}");
        self.__goal_flush();
        self.__mission_flush();
    ...
    def calculate_mission_total_distance(self) -> None:
        try:
            self.__log.info(f"{ROUTE_TO_POSE_ACTION_NAME} calculate mission_distance :  {self.mission_start_distance} / {self.__current_distance}");
            self.__mission_end_distance = self.__current_distance;
            mission_total_distance: int = int(abs(self.__mission_end_distance - self.mission_start_distance));
        ...
        if result_code == 1001:
            ...
            if is_end_node_is_mission_source:
                ...
                if get_is_mission_canceled() is True:
                    ...
                    self.__log.info(f"==================================== Source Arrived ===================================={self.mission_start_distance}");
                    self.__goal_flush();
                    return;
                else:
                    pass;
                ...
                if not is_mission_returning_task and not get_is_mission_canceled():
                    ...
                    self.__log.info(f"==================================== Dest Arrived ===================================={self.mission_start_distance}");
                    self.__goal_flush();
                    return;
                ...
                if path_response != None or len(path_response.path.node_list) != 0:
                    ...
                    return;
            elif get_is_mission_canceled() and self.__current_goal.end_node.node_id == self.__goal_list[self.__goal_list_size - 1].node_id and not is_mission_returning_task:
                set_driving_flag(flag=False);
                set_driving_status(driving_status=DRIVE_STATUS_WAIT);
