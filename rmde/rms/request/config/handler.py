...
class ConfigRequestHandler():
    ...
    def __init__(self, rclpy_node: Node, mqtt_client: Client) -> None:
        ...
        self.__mqtt_config_subscription_topic: str = self.__mqtt_config_parser.get(
            'topics', 'config')
        self.__mqtt_config_subscription_qos: int = int(
            self.__mqtt_config_parser.get('qos', 'config'))
    ...
    def request_to_uvc(self) -> None:
        def __mqtt_config_subscription_cb(mqtt_client: mqtt.Client, mqtt_user_data: Dict, mqtt_message: mqtt.MQTTMessage) -> None:
            try:
                ...
                self.__common_config_parser.set(
                    'header', 'robotType', self.__set_info.robotType)
                self.__common_config_parser.set(
                    'header', 'robotCorpId', self.__set_info.robotCorpId)
                self.__common_config_parser.set(
                    'header', 'robotId', self.__set_info.robotId)
                self.__common_config_parser.set(
                    'header', 'workCorpId', self.__set_info.workCorpId)
                self.__common_config_parser.set(
                    'header', 'workSiteId', self.__set_info.workSiteId)

                self.__mqtt_config_parser.set(
                    'broker', 'host', self.__set_info.mqttIP)
                self.__mqtt_config_parser.set(
                    'broker', 'port', self.__set_info.mqttPort)
                ...
                self.__mqtt_config_parser.set(
                    'topics', 'path', modified_path_topic)
                self.__mqtt_config_parser.set(
                    'topics', 'control', modified_control_topic)
                self.__mqtt_config_parser.set(
                    'topics', 'config', modified_config_topic)
                ...
                with open(self.__common_config_service.config_file_path, 'w') as common_config_file:
                    ...
                    self.__rclpy_node.get_logger().info(
                        '===== Config Common Configuration has been changed =====')
                ...
                with open(self.__mqtt_config_service.config_file_path, 'w') as mqtt_config_file:
                    ...
                    self.__rclpy_node.get_logger().info(
                        f'===== Config MQTT Configuration has been changed with IP [{self.__set_info.mqttIP}] reboot required =====')
                    self.__rclpy_node.destroy_node()

            except KeyError as ke:
                self.__rclpy_node.get_logger().error(
                    f'Invalid JSON Key in MQTT {self.__mqtt_config_subscription_topic} subscription callback: {ke}')

            except json.JSONDecodeError as jde:
                self.__rclpy_node.get_logger().error(
                    f'Invalid JSON format in MQTT {self.__mqtt_config_subscription_topic} subscription callback: {jde.msg}')

            except Exception as e:
                self.__rclpy_node.get_logger().error(
                    f'Exception in MQTT {self.__mqtt_config_subscription_topic} subscription callback: {e}')
                raise

        self.__mqtt_client.subscribe(
            topic=self.__mqtt_config_subscription_topic, qos=self.__mqtt_config_subscription_qos)
        self.__mqtt_client.client.message_callback_add(
            sub=self.__mqtt_config_subscription_topic, callback=__mqtt_config_subscription_cb)
