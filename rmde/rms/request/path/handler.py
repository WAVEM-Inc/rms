...
class PathRequestHandler():
    ...
    def __init__(self, rclpy_node: Node, mqtt_client: Client) -> None:
        ...
        self.__mqtt_path_subscription_topic: str = self.__mqtt_config_parser.get(
            'topics', 'path')
        self.__mqtt_path_subscription_qos: int = int(
            self.__mqtt_config_parser.get('qos', 'path'))
        ...
        def __mqtt_path_subscription_cb(mqtt_client: mqtt.Client, mqtt_user_data: Dict, mqtt_message: mqtt.MQTTMessage) -> None:
            try:
                ...
                self.__rclpy_node.get_logger().info(
                    f'{MQTT_FLAG} subscription cb payload [{mqtt_decoded_payload}] from [{mqtt_topic}]')
                self.__rclpy_node.get_logger().info(
                    f'{MQTT_FLAG} subscription cb json [{mqtt_decoded_payload}] from [{mqtt_topic}]')
                ...
                self.__cmd_repsonse_handler.response_to_rms(
                ...
                rclpy_request_register_uuid_service_result: Any = self.__rclpy_register_task_service_request(
                    job_uuid)
                self.__rclpy_node.get_logger().info(
                    f'PathRequestHandler request_register_uuid_result : {rclpy_request_register_uuid_service_result}')
                ...
                self.__rclpy_node.get_logger().info(
                    f'PathRequestHandler LocationList {self.__jobPath.locationList}')
                self.__rclpy_publish_goal_waypoints_list(
                    self.__jobPath.locationList)

            except KeyError as ke:
                self.__rclpy_node.get_logger().error(
                    f'PathRequestHandler Invalid JSON Key in MQTT {self.__mqtt_path_subscription_topic} subscription callback: {ke}')

            except json.JSONDecodeError as jde:
                self.__rclpy_node.get_logger().error(
                    f'PathRequestHandler Invalid JSON format in MQTT {self.__mqtt_path_subscription_topic} subscription callback: {jde.msg}')

            except Exception as e:
                self.__rclpy_node.get_logger().error(
                    f'PathRequestHandler Exception in MQTT {self.__mqtt_path_subscription_topic} subscription callback: {e}')
                raise

        self.__mqtt_client.subscribe(
            topic=self.__mqtt_path_subscription_topic, qos=self.__mqtt_path_subscription_qos)
        self.__mqtt_client.client.message_callback_add(
            sub=self.__mqtt_path_subscription_topic, callback=__mqtt_path_subscription_cb)

    def __rclpy_lookup_messages(self, module_name: str, module_class_name: str) -> Any:
        self.__rclpy_node.get_logger().info(
            f'{RCLPY_FLAG} lookup object module_name : {module_name}')
        self.__rclpy_node.get_logger().info(
            f'{RCLPY_FLAG} lookup object module_class_name : {module_class_name}')

        message_path = importlib.import_module(
            module_name, self.__rclpy_node.get_name())
    ...
    def __rclpy_register_task_service_request(self, job_uuid: JobUUID) -> Any:
        self.__rclpy_node.get_logger().info(
            f'PathRequestHandler request_register_uuid job_plan_id : {job_uuid.jobPlanId}')
        self.__rclpy_node.get_logger().info(
            f'PathRequestHandler request_register_uuid job_group_id : {job_uuid.jobGroupId}')
        self.__rclpy_node.get_logger().info(
            f'PathRequestHandler request_register_uuid job_order_id : {job_uuid.jobOrderId}')
    ...
    def __rclpy_publish_goal_waypoints_list(self, location_list: list) -> None:
        ...
        for location in location_list:
            rclpy_nav_sat_fix_obj: Any = self.__rclpy_lookup_messages(
            ...
            rclpy_nav_sat_fix: Any = message_conversion.populate_instance(
                rclpy_location_to_nav_sat_fix_dict, rclpy_nav_sat_fix_obj())
            rclpy_goal_waypoints_list.append(rclpy_nav_sat_fix)

        self.__rclpy_node.get_logger().info(
            'GoalWaypoints List : {}'.format(rclpy_goal_waypoints_list))

        rclpy_goal_waypoints: GoalWaypoints = GoalWaypoints()
        rclpy_goal_waypoints.goal_waypoints_list = rclpy_goal_waypoints_list

        self.__rclpy_node.get_logger().info(
            'GoalWaypoints : {}'.format(rclpy_goal_waypoints.goal_waypoints_list))
